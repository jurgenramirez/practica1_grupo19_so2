<template>
  <v-data-table
    :headers="columnas"
    :items="filas"
    :search="search"
    :loading="loading"
    class="elevation-1"
    reactive
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>{{ nombre_tabla }}</v-toolbar-title>
        <v-divider class="mx-auto" inset vertical></v-divider>
        <v-spacer></v-spacer>
        <v-dialog v-model="dialog" max-width="600px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              color="primary"
              dark
              class="mb-2 m-3 rounded-pill"
              v-bind="attrs"
              v-on="on"
            >
              Registrar nuevo usuario
            </v-btn>
            <v-text-field
              v-model="search"
              append-icon="mdi-magnify"
              label="Buscar Usuario"
              single-line
              hide-details
            ></v-text-field>
          </template>

          <v-card>
            <v-card-title>
              <span class="text-h5">{{ formTitle }}</span>
            </v-card-title>
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="6" lg="12">
                    <v-text-field
                      v-if="editedIndex === -1"
                      v-model="editedItem.dpi"
                      label="CUI"
                      :counter="13"
                      :rules="cuiRules"
                      type="number"
                      required
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-text-field
                      v-model="editedItem.nombre"
                      label="Nombre"
                      :rules="nameRules"
                      required
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-text-field
                      v-model="editedItem.apellido"
                      label="Apellido"
                      :rules="apellidoRules"
                      required
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-autocomplete
                      :items="secciones_nombre"
                      v-model="editedItem.seccion"
                      label="Sección"
                      :rules="campoRules"
                      color="warning"
                      required
                    ></v-autocomplete>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-text-field
                      v-model="editedItem.email"
                      label="Email"
                      :placeholder="'example@mem.gob.gt'"
                      :rules="emailRules"
                      required
                    ></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-autocomplete
                      :items="puestos_nombre"
                      v-model="editedItem.puesto"
                      label="Puesto"
                      :rules="campoRules"
                      color="warning"
                      required
                    ></v-autocomplete>
                  </v-col>
                  <v-col cols="12" sm="6" md="12" lg="12">
                    <v-autocomplete
                      :items="isAdmin"
                      v-model="editedItem.admin"
                      label="Es administrador"
                      :rules="campoRules"
                      color="warning"
                    ></v-autocomplete>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="red darken-1" text @click="close"> Cancelar </v-btn>
              <v-btn color="blue darken-1" text @click="save"> Guardar </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-toolbar>
    </template>
    <template v-slot:[`item.actions`]="{ item }">
      <v-icon
        small
        class="rounded p-2 bg-warning m-1 text-white rounded-pill"
        data-toggle="tooltip"
        title="Editar Usuario"
        @click="editItem(item)"
      >
        mdi-pencil
      </v-icon>
      <v-icon
        v-if="item.deleted_at == null"
        small
        class="rounded p-2 bg-danger m-1 text-white rounded-pill"
        data-toggle="tooltip"
        title="Desactivar Usuario"
        @click="deleteItem(item)"
      >
        mdi-delete
      </v-icon>
      <v-icon
        v-if="item.deleted_at != null"
        small
        class="rounded p-2 bg-success m-1 text-white rounded-pill"
        data-toggle="tooltip"
        title="Activar Usuario"
        @click="deleteItem(item)"
      >
        mdi-account-reactivate
      </v-icon>
    </template>
    <template v-slot:no-data>
      <v-btn color="primary"> Reset </v-btn>
    </template>
  </v-data-table>
</template>

<script src="./Tabla_mantenimientos_usuario.js"></script>