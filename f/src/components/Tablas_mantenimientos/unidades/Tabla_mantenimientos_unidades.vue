<template>
  ...
  >
    <template v-slot:top>
      ...
      >
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5">¿Seguro de eliminar el dato seleccionado?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
    ...
    <template v-slot:[`item.actions`]="{ item }">
      <v-icon
        small
        class="rounded-pill m-1 p-2 bg-warning text-white"
        data-toggle="tooltip" title="Editar Usuario"
        @click="editItem(item)"
      >
        mdi-pencil
      </v-icon>
      <!-- No se pueden desactivar unidades
      <v-icon
        v-if="item.deleted_at == null"
        small
        class="rounded-pill m-1 p-2 bg-danger text-white"
      ...
      <v-icon
        v-if="item.deleted_at != null"
        small
        class="rounded-pill m-1 p-2 bg-success text-white"
...
<script src="./Tabla_mantenimientos_unidades.js"></script>